...
static  IplImage		*dst, *out, *temp;										//image variables for processing
static IplImage		*tmp1, *tmp2;
static IplImage		*Bimg, *Rimg, *Gimg, *mask, *merge;						//image variables for splitting, masking and merging
static IplImage		*regions[9];											// 9 image regions through which the plane will pass

unsigned long	min_region_cost;										//	minimum cost of the region through which plane can pass
static double			horizon_a,horizon_b, horizon_x0, horizon_y0;			// varibles for hough transform
static float			*horizon_line;
static float			horizon_rho, horizon_theta;
static float			area, horizon_angle;									// obstacle area and horizon line angle

static CvPoint			horizon_pt1, horizon_pt2, horizon_p1, horizon_p2;		//variables for horizon
static CvPoint			poly[4], centroid;										// centroid of obstacles and fill poly polygon.
static CvPoint			rod[4]={{320,0}, {374,0}, {367,370}, {323,370}};		// erasing the rod
static CvPoint			final_pt[4] = {{0, FRAME_HEIGHT/2}, {FRAME_WIDTH-1, FRAME_HEIGHT/2}, {FRAME_WIDTH-1, FRAME_HEIGHT-1}, {0, FRAME_HEIGHT-1}};	//below horizon

static CvRect			horizon_r;												// obstacle bounding rect



//	memory storage and seqence pointers for contours and hough transform
static CvMemStorage	*horizon_storage = cvCreateMemStorage(0);
static CvMemStorage	*contour_storage = cvCreateMemStorage(0);
static CvSeq			*horizon_lines;
static CvSeq			*contours, *ptr;



//	pointers for list traversal and cost for error functions and heading
static unsigned char	*basePtr, *iterator;
static unsigned long	current_cost = 0, cost = 0xFFFFFFFF;
static unsigned int	count = 0, heading_region;
...
extern void skySegment(IplImage *frame)
{
...
extern bool selectHorizonLineByCost(IplImage* img, CvPoint p1, CvPoint p2)
{
	cvCopy(img, tmp1);
	cvCopy(img, tmp2);
	poly[0] = p1;
...
static CvPoint findIntersect(CvRect A, CvRect B)
{
	CvPoint delta = {0,0};
